  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = isEditing
        ...
        : await fetch('http://localhost:2030/upload-subcategory', {
            method: 'POST',
            body: formData,
          });
      ...
      if (response.ok) {
        ...
        const dataResponse = await axios.get('http://localhost:2030/fetch-subcategories');
          ...
          <Modal open={open} onClose={handleClose} aria-labelledby="modal-title" aria-describedby="modal-description">
            <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white shadow-lg rounded-lg p-6 w-96">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <TextField label="Sub-category Name" fullWidth variant="outlined" value={subCategoryName} onChange={(e) => setSubCategoryName(e.target.value)} required />
                ...
                <TextField label="Sequence" type="number" fullWidth variant="outlined" value={categorySequence} onChange={(e) => setCategorySequence(parseFloat(e.target.value) || '')} required />
